...
public class AssessmentService {
    ...
    public Quiz createQuiz(Quiz quiz, String type, int nQuestions) {
        if (assessmentRepository.findByTitle(quiz.getTitle()).isPresent()) {
            throw new RuntimeException("Quiz title already exists");
        }
        ...
        // quiz.setQuestions(questionService.getRandomizedQuestions(quiz,
        // quiz.getCourseTitle(), type, nQuestions));
        // quiz.setQuestions(
        // questionService.getRandomizedQuestions(quiz, quiz.getCourseTitle(), type,
        // nQuestions)
        // .stream()
        // .map((question, index) -> {
        // question.setQuestionOrder(index + 1); // Explicitly set questionOrder
        // return question;
        // })
        // .collect(Collectors.toList()));
        // quiz.setCourse(course);
        List<Question> questions = questionService.getRandomizedQuestions(quiz, quiz.getCourseTitle(), type, nQuestions);
